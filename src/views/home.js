...
async function loadRecipes() {
    const {results: recipe} = await getRecipes();

    if(recipe.length == 0 ) {
        return html`<p style="text-align: center">No recipes found. Be the first to post a recipe!</p>`
    } else {
        return recipe.reduce((a, c) => {
            if(a.length > 0) {
